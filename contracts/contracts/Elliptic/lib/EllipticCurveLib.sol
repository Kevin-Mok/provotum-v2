...
library EllipticCurveLib {
    ...
    function expMod(uint256 _base, uint256 _exp, uint256 _pp) public pure returns (uint256) {
        if (_base == 0) return 0;
        if (_exp == 0) return 1;
        if (_pp == 0) revert("Modulus is zero");
        uint256 r = 1;
        uint256 bit = 2**255;
    ...
    function toAffine(uint256 _x, uint256 _y, uint256 _z, uint256 _pp) public pure returns (uint256, uint256) {
    ...
    function deriveY(uint8 _prefix, uint256 _x, uint256 _aa, uint256 _bb, uint256 _pp) public pure returns (uint256) {
        // x^3 + ax + b
    ...
    function isOnCurve(uint256 _x, uint256 _y, uint256 _aa, uint256 _bb, uint256 _pp) public pure returns (bool) {
        ...
        returns (uint256, uint256, uint256)
    {
        ...
        // qy = -8*y1^4 + M(S-T)
        uint256 qy = addmod(
            mulmod(m, addmod(s, _pp - qx, _pp), _pp),
            _pp - mulmod(8, mulmod(square[1], square[1], _pp), _pp),
            _pp
        );
        // qz = 2*y1*z1
